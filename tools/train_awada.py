...
def _train(cfg: DictConfig) -> None:
    """Run AWADA training from a Hydra config."""
    ...
    dataset = AttentionPairedDataset(
        cfg.data.source_dir,
        cfg.data.target_dir,
        cfg.data.source_attention_dir,
        target_attention_root=cfg.data.target_attention_dir,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        ckpt_path = os.path.join(cfg.data.output_dir, f"awada_epoch_{epoch + 1}.pth")
        if (epoch + 1) % cfg.training.save_every == 0 or (epoch + 1) == epochs:
            torch.save(ckpt, ckpt_path)
            logger.info("Checkpoint saved: %s", ckpt_path)

    logger.info("AWADA training complete.")


@hydra.main(version_base=None, config_path="../configs", config_name="train_awada")
