...
def _train(cfg: DictConfig) -> None:
    """Run CycleGAN training from a Hydra config."""
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for iteration, (real_A, real_B) in enumerate(
            ...
            with torch.cuda.amp.autocast(enabled=cfg.training.amp):
                g_losses = model.compute_generator_loss(
                    cfg.model.lambda_cyc, cfg.model.lambda_gan, cfg.model.lambda_idt
                )
        ...
        ckpt_path = os.path.join(cfg.data.output_dir, f"cyclegan_epoch_{epoch + 1}.pth")
...
@hydra.main(version_base=None, config_path="../configs", config_name="train_cyclegan")
